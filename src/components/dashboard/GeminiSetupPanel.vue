...
import type {
  ...
  SaveGeminiCredentialsInput,
  SaveGeminiCredentialsResult,
} from '../../types/usage'
...
const connection = shallowRef<GeminiConnectionState | null>(null)
...
async function loadConnection() {
  try {
    connection.value = await invoke<GeminiConnectionState>('get_gemini_connection')
    form.accountLabel = connection.value.accountLabel ?? ''
  } catch (error) {
    errorMessage.value =
      error instanceof Error ? error.message : 'Could not read Gemini configuration'
  }
}
...
async function saveCredentials() {
  ...
  try {
    const payload: SaveGeminiCredentialsInput = {
      apiKey: form.apiKey,
      accountLabel: form.accountLabel || null,
    }

    const result = await invoke<SaveGeminiCredentialsResult>('save_gemini_credentials', {
      input: payload,
    })
...
async function syncGemini() {
  ...
  try {
    ...
    message.value =
      'Sync requested. Gemini limits and historical spend require usageMetadata capture or Cloud Billing.'
    emit('updated')
  } catch (error) {
    errorMessage.value = error instanceof Error ? error.message : 'Could not sync Gemini'
  } finally {
    isLoading.value = false
  }
}
...
<template>
  <div v-if="isModal" class="mt-4">
    <p class="text-xs leading-5 text-ledger-muted">
      The AI Studio key is stored in the system keyring. Google validates access through the Models API, but historical spend and remaining AI Studio limits are not exposed through a public endpoint.
    </p>

    <form class="mt-5 grid gap-3" @submit.prevent="saveCredentials">
      <label class="grid gap-2">
        <span class="text-xs text-ledger-muted">API key</span>
        <input v-model="form.apiKey" class="min-h-10 rounded-xl border border-ledger-line bg-ledger-paper px-3 text-sm text-ledger-ink outline-none transition focus:border-ledger-brass" placeholder="AIza..." type="password" autocomplete="off" />
      </label>
      ...
      <div class="flex flex-col gap-3 pt-1 sm:flex-row">
        ...
        <button class="inline-flex min-h-10 items-center justify-center gap-2 rounded-xl border border-ledger-line bg-ledger-paper px-3 text-xs font-semibold text-ledger-ink transition hover:bg-ledger-inset disabled:cursor-not-allowed disabled:opacity-60" :disabled="isLoading || !connection?.hasCredentials" type="button" @click="syncGemini">
          <RefreshCw :class="['size-4', isLoading && 'animate-spin']" aria-hidden="true" /> Sync Gemini
        </button>
      </div>
    </form>
  ...
  <section v-else class="rounded-[1.5rem] border border-ledger-line bg-ledger-panel p-4" aria-labelledby="gemini-title">
    <div class="flex items-start justify-between gap-3">
      <div>
        <p class="font-mono text-[0.65rem] uppercase tracking-[0.18em] text-ledger-muted">AI Studio key</p>
        <h2 id="gemini-title" class="text-sm font-semibold text-ledger-ink">Connect Gemini</h2>
      </div>
