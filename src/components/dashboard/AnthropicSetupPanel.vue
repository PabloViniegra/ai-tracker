...
<template>
  <section class="rounded-3xl border border-slate-800 bg-slate-900/60 p-5" aria-labelledby="anthropic-title">
    <div class="flex items-start justify-between gap-3">
      <div>
        <p class="font-mono text-xs uppercase tracking-[0.22em] text-slate-500">Vault + SQLite</p>
        <h2 id="anthropic-title" class="mt-1 text-xl font-semibold text-slate-50">Conectar Anthropic</h2>
      </div>
    ...
    <p class="mt-4 text-sm leading-6 text-slate-400">
      La API key se guarda en el keyring del sistema. La metadata queda en SQLite local.
      Anthropic no expone un endpoint publico de usage para cuentas normales; el sync reflejara esta limitacion.
    </p>

    <form class="mt-5 grid gap-3" @submit.prevent="saveCredentials">
      <label class="grid gap-2">
        <span class="text-sm text-slate-300">API key</span>
        <input v-model="form.apiKey" class="min-h-11 rounded-xl border border-slate-700 bg-slate-950 px-3 text-slate-100 outline-none transition focus:border-amber-400" placeholder="sk-ant-..." type="password" autocomplete="off" />
      </label>
      ...
      <div class="flex flex-col gap-3 pt-1 sm:flex-row">
        ...
        <button class="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-slate-700 bg-slate-950 px-4 text-sm font-semibold text-slate-100 transition hover:border-blue-400/40 hover:text-blue-100 disabled:cursor-not-allowed disabled:opacity-60" :disabled="isLoading || !connection?.hasCredentials" type="button" @click="syncAnthropic">
          <RefreshCw :class="['size-4', isLoading && 'animate-spin']" aria-hidden="true" /> Sincronizar Anthropic
        </button>
      </div>
    </form>
