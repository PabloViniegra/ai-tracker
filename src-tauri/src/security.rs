...
pub fn save_gemini_api_key(service: &str, api_key: &str) -> Result<(), String> {
    let entry = Entry::new(service, GEMINI_ACCOUNT).map_err(|error| error.to_string())?;
    entry
        .set_password(api_key)
        .map_err(|error| error.to_string())
}

pub fn load_gemini_api_key(service: &str) -> Result<Option<String>, String> {
    let entry = Entry::new(service, GEMINI_ACCOUNT).map_err(|error| error.to_string())?;
    match entry.get_password() {
        Ok(secret) => Ok(Some(secret)),
        Err(keyring::Error::NoEntry) => Ok(None),
